...
enum TokenKind {
    ...
    // Reserved words.
    ABSTRACT("abstract"), BOOLEAN("boolean"), BREAK("break"), CASE("case"),
    CATCH("catch"), CLASS("class"), CONTINUE("continue"), DEFLT("default"), DO("do"),
    DOUBLE("double"), ELSE("else"), EXTENDS("extends"), FINALLY("finally"), FOR("for"),
    CHAR("char"), IF("if"), IMPLEMENTS("implements"), IMPORT("import"),
    INSTANCEOF("instanceof"), INT("int"), INTERFACE("interface"), LONG("long"),
    NEW("new"), PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"),
    PUBLIC("public"), RETURN("return"), STATIC("static"), SUPER("super"),
    SWITCH("switch"), THIS("this"), THROW("throw"), THROWS("throws"), TRY("try"),
    VOID("void"), WHILE("while"),

    // Operators.
    ALSHIFT("<<"), ALSHIFT_ASSIGN("<<="), AND("&"), AND_ASSIGN("&="), ARSHIFT(">>"),
    ARSHIFT_ASSIGN(">>="), ASSIGN("="), COLON(":"), DEC("--"), DIV("/"),
    DIV_ASSIGN("/="), EQUAL("=="), GE(">="), GT(">"), INC("++"), LAND("&&"),
    LE("<="), LNOT("!"), LOR("||"), LRSHIFT(">>>"), LRSHIFT_ASSIGN(">>>="),
    LT("<"), MINUS("-"), MINUS_ASSIGN("-="), NOT("~"), NOT_EQUAL("!="), OR("|"),
    OR_ASSIGN("|="), PLUS("+"), PLUS_ASSIGN("+="), QUESTION("?"), REM("%"),
    REM_ASSIGN("%="), STAR("*"), STAR_ASSIGN("*="), XOR("^"), XOR_ASSIGN("^="),
