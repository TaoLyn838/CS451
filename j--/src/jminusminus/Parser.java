...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        ArrayList<TypeName> superInterfaces = handleQualifiedIdentifier(IMPLEMENTS);
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        mustBe(IDENTIFIER);
        String name = scanner.previousToken().image();
        ArrayList<TypeName>  superInterfaces = handleQualifiedIdentifier(EXTENDS);
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            ArrayList<TypeName> exceptions = handleQualifiedIdentifier(THROWS);
            JBlock body = block();
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods) {
        int line = scanner.token().line();
        if (have(CLASS) || have(INTERFACE)) {
            reportParserError("Line: %d. Interface can no have either inner classes or interface!", line);
            return null;
        }
        ...
        Type type;
        if (have(VOID)) {
            // Complete the modifiers when a class implements an interface
            // ex: class FactorialIter implements interfaceClass { public int compute(int n) }
            //          -> "modifiers": ["public", "abstract"], "returnType": "int", "name": "compute",
            //          NOT return "modifiers": ["public"] !!!
            if (!mods.contains("abstract")) {
                mods.add("abstract");
            }
            // All abstract, default, and static methods in an interface are implicitly public!!!
            // ex: class FactorialIter implements interfaceClass { int compute(int n) }
            //          -> "modifiers": ["public", "abstract"], "returnType": "int", "name": "compute",
            //          NOT return "modifiers": ["abstract"] !!!
...
//        | FOR LPAREN [ forInit ] SEMI [ expression ] SEMI [ forUpdate ] RPAREN statement
            ...
            // create all return variables for parameters in JForStatement().
            ...
            // initialize ArrayList<JStatement> init and parsing "for ( variable" while we don't see ';'
            init = see(SEMI) ? null : forInit();
            // for ( variable;
            mustBe(SEMI);

            // for ( variable; expression
            // ex: ( int a = 0; a < 10;
            condition = see(SEMI) ? null : expression();
            mustBe(SEMI);

            // for ( variable; expression; update)
            update = see(RPAREN) ? null : forUpdate();
            mustBe(RPAREN);
...
//      TRY block { CATCH LPAREN formalParameter RPAREN block } [ FINALLY block ]
    ...
    /**
     * Parses a for loop initializations and returns an AST for it.
     *
     * <pre>
    ...
    private ArrayList<JStatement> forInit() {
        ArrayList<JStatement> initList = new ArrayList<>();

        // check if here has a local variable declaration
        if (seeLocalVariableDeclaration()) {
            int line = scanner.token().line();
            ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
            initList.add(new JVariableDeclaration(line, vdecls));
            // return a list containing a single local variable
            return initList;
        }

        // Otherwise, variableDeclarators ::= variableDeclarator { COMMA variableDeclarator }
        do {
            initList.add(statementExpression());
        } while (have(COMMA));
    ...
    private ArrayList<JStatement> forUpdate() {
        ArrayList<JStatement> updatedStatement = new ArrayList<>();
        do {
            updatedStatement.add(statementExpression());
        } while (have(COMMA));
    ...
    /**
     * Parses a switch block statement group for switch statement.
     *
     * <pre>
     *   switchBlockStatementGroup ::= switchLabel { switchLabel } { blockStatement }
     * </pre>
     *
     * @return a group of switch statement.
     */
    ...
    private SwitchStatementGroup switchBlockStatementGroup() {
        ...
        // Add base switchLabel, then keep add zero or more switchLabels while parser doesn't see 'case' | `default`
        do {
            switchLabels.add(switchLabel());
        } while (see(CASE) || see(DEFLT));

        // Adding zero or more blockStatement while parser doesn't see 'case' and `default' and '}'
    ...
    /**
     * Parses a switch label for switch statement.
     *
     * <pre>
    ...
    private JExpression switchLabel() {
        ...
        if(have(CASE)) {
            ...
            reportParserError("Unexpected Type sought where %s found in the switch statement",
                    scanner.token().image());
        }
     ...
     *                                [ ( ALSHIFT_ASSIGN | AND_ASSIGN | ARSHIFT_ASSI GN | ASSIGN | DIV_ASSIGN
    ...
    /**
     * Parses a conditional expression and return an AST for it.
     *
     * <pre>
     *
    ...
    private JExpression relationalExpression() {
        ...
        // Adding `>=` and `<` to Completed relationalExpression()
        if (have(GE)) {
    ...
    private JExpression postfixExpression() {
        ...
        // Improvement postfixExpression that can parser { DEC | INC }
        while (true) {
            if (have(DEC)) {
    ...
    private JExpression literal() {
        ...
        // Adding DOUBLE_LITERAL and LONG_LITERAL
    ...
    //////////////////////////////////////////////////
    // Additional helper parser methods
    //////////////////////////////////////////////////

    /**
     * A helper parse method that the object owner (Chengtao) wrote, DOES NOT include in the j-- grammar rule.
     * It is useful in class and interface declaration to handle exception and implement interface.
     *
     * <pre>
     *   handleQualifiedIdentifier ::= [ sought qualifiedIdentifier { COMMA qualifiedIdentifier } ]
     * </pre>
     *
     * @param sought the TokenKind
     * @return an array list of TypeName.
     */

    private ArrayList<TypeName> handleQualifiedIdentifier(TokenKind sought) {
        ArrayList<TypeName> qualifiedIdList = null;

        if (have(sought)) {
            qualifiedIdList = new ArrayList<>();
            do {
                qualifiedIdList.add(qualifiedIdentifier());
            } while (have(COMMA));
        }
